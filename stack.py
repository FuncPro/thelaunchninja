...
redirect_bucket = t.add_resource(
    Bucket("RedirectBucket",
           BucketName='thelaunchninja.com',
           AccessControl=PublicRead,
           WebsiteConfiguration=WebsiteConfiguration(
               RedirectAllRequestsTo=RedirectAllRequestsTo(
                   HostName=Ref(root_bucket)
...
redirect_bucket = t.add_resource(
    Bucket("WWWRedirectBucket",
           BucketName='www.thelaunchninja.com',
           AccessControl=PublicRead,
           WebsiteConfiguration=WebsiteConfiguration(
               RedirectAllRequestsTo=RedirectAllRequestsTo(
                   HostName=Ref(root_bucket)
...
redirect_record = t.add_resource(RecordSetGroup(
    'RedirectRecordSetGroup',
    HostedZoneName='thelaunchninja.com.',
    RecordSets=[
        RecordSet(
            Name='thelaunchninja.com',
            Type='A',
            AliasTarget=AliasTarget(
                hostedzoneid=FindInMap('RegionMap', Ref('AWS::Region'), 'S3hostedzoneID'),
                dnsname=FindInMap('RegionMap', Ref('AWS::Region'), 'websiteendpoint'),
        ...
        RecordSet(
            Name='www.thelaunchninja.com',
            Type='CNAME',
            TTL='900',
            ResourceRecords=[
                Join('.', ['www.thelaunchninja.com', FindInMap('RegionMap', Ref('AWS::Region'), 'websiteendpoint')])
            ]
        ),
    ]
))
